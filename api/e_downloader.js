...
const DATASET_DIR = '../dataset/employers';
...
function findExistingOutputFile(startId) {
  const files = fs.readdirSync(DATASET_DIR).filter(file =>
    file.endsWith('.json') && file.startsWith('hh_employers_')
  );
  ...
  for (const file of files) {
    ...
    if (stats.size < MAX_FILE_SIZE) {
      const match = file.match(/hh_employers_(\d+)_to_(\d+)\.json/);
      if (match) {
...
function createNewOutputFile(startId, endId) {
  const fileName = `hh_employers_${startId}_to_${endId}.json`;
...
async function main() {
  ...
  console.log(`Всего работодателей для обработки: ${count}`);
